...
def back_track(destination_node):
    ...
    res_str = ""  # 리턴할 결과 문자열
    backtracking_node = destination_node    #  도착 노드에서 시작 노드까지 찾아가는 데 사용할 변수
    
    # 시작 노드까지 갈 때까지
    while backtracking_node:
        res_str = f"{backtracking_node.station_name} " + res_str    # 결과 문자열에 역 이름을 더하고
        backtracking_node = backtracking_node.predecessor   # backtracking_node를 다음 노드로 바꿔준다
