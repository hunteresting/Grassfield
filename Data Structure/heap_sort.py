...
def heapsort(tree):
    ...
    for index in range(1, tree_size-1):
        for compare in range(index, tree_size):
            if tree[index] < tree[compare]:
                swap(tree, index, compare)
                heapify(tree, 1, tree_size)
    new_tree = tree[len(tree):0:-1]
    new_tree.insert(0, None)
